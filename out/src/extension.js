...
var Logger = (function () {
  ...
  Logger.showInformationMessage = function (message) {
    ...
    this.log(message);
    return (_a = vscode.window).showInformationMessage.apply(
      _a,
  ...
  Logger.showErrorMessage = function (message) {
    ...
    this.log(message);
    return (_a = vscode.window).showErrorMessage.apply(
      _a,
      [message].concat(items)
    );
    var _a;
...
var Paster = (function () {
  ...
  Paster.paste = function () {
    ...
    this.defaultNameConfig = vscode.workspace.getConfiguration('pasteImage')[
      'defaultName'
    ];
    ...
    this.folderPathConfig = vscode.workspace.getConfiguration('pasteImage')[
      'path'
    ];
    ...
    this.basePathConfig = vscode.workspace.getConfiguration('pasteImage')[
      'basePath'
    ];
    ...
    // load other config
    this.prefixConfig = vscode.workspace.getConfiguration('pasteImage')[
      'prefix'
    ];
    this.suffixConfig = vscode.workspace.getConfiguration('pasteImage')[
      'suffix'
    ];
    this.forceUnixStyleSeparatorConfig = vscode.workspace.getConfiguration(
      'pasteImage'
    )['forceUnixStyleSeparator'];
    this.forceUnixStyleSeparatorConfig = !!this.forceUnixStyleSeparatorConfig;
    this.encodePathConfig = vscode.workspace.getConfiguration('pasteImage')[
      'encodePath'
    ];
    this.namePrefixConfig = vscode.workspace.getConfiguration('pasteImage')[
      'namePrefix'
    ];
    this.nameSuffixConfig = vscode.workspace.getConfiguration('pasteImage')[
      'nameSuffix'
    ];
    this.insertPatternConfig = vscode.workspace.getConfiguration('pasteImage')[
      'insertPattern'
    ];
    this.showFilePathConfirmInputBox =
      vscode.workspace.getConfiguration('pasteImage')[
        'showFilePathConfirmInputBox'
      ] || false;
    this.filePathConfirmInputBoxMode = vscode.workspace.getConfiguration(
      'pasteImage'
  ...
  Paster.saveAndPaste = function (editor, imagePath) {
    ...
    this.createImageDirWithImagePath(imagePath)
      .then(function (imagePath) {
        ...
        _this.saveClipboardImageToFileAndGetPath(imagePath, function (
          imagePath,
          ...
          imagePath = _this.renderFilePath(
            editor.document.languageId,
            _this.basePathConfig,
            imagePath,
            _this.forceUnixStyleSeparatorConfig,
            _this.prefixConfig,
            _this.suffixConfig
          );
  ...
  Paster.getImagePath = function (
    ...
    if (
      filePathConfirmInputBoxMode ==
      Paster.FILE_PATH_CONFIRM_INPUTBOX_MODE_PULL_PATH
    ) {
      ...
      vscode.window
        ...
        .then(function (result) {
          if (result) {
            ...
            if (
              filePathConfirmInputBoxMode ==
              Paster.FILE_PATH_CONFIRM_INPUTBOX_MODE_ONLY_NAME
            ) {
      ...
      fs.stat(imageDir, function (err, stats) {
        if (err == null) {
          if (stats.isDirectory()) {
            ...
            reject(
              new PluginError(
                "The image dest directory '" +
                  imageDir +
                  "' is a file. please check your 'pasteImage.path' config."
              )
            );
          }
   ...
   */
  Paster.renderFilePath = function (
    languageId,
    basePath,
    imageFilePath,
    forceUnixStyleSeparator,
  ...
  Paster.replacePathVariable = function (
    pathStr,
    projectRoot,
    curFilePath,
    postFunction
  ) {
    if (postFunction === void 0) {
    ...
    pathStr = pathStr.replace(
      this.PATH_VARIABLE_CURRNET_FILE_DIR,
